...
* ``cdcs input.py [--out report.json]`` (default) — analyzer/reporter,
  same behavior as the original POC: parses, validates, emits JSON.
* ``cdcs compile input.py [--dest dir/]`` — full synthesis: invokes
  the LLM, emits ``input.generated.py`` / ``test_input.generated.py``,
  updates ``cdcs.lock``.
* ``cdcs check input.py [--dest dir/]`` — CI mode: no LLM call;
...
from cdcs.cli.parsers import (
    SUBCOMMANDS,
    build_check_parser,
    build_compile_parser,
    build_parser,
)
from cdcs.cli.ui import ConsoleUI, has_any_diagnostic
...
from cdcs.synthesis.llm import LLMClient, LLMError, default_llm_client
...
def select_adapter(path: Path) -> LanguageAdapter:
    ...
    recognised, matching cdcs's original Python-only behaviour for
...
def _read_source(input_path: Path, ui: ConsoleUI) -> str | None:
    if not input_path.is_file():
        ui.fatal(f"cdcs: input file not found: {input_path}")
        return None
    try:
        return input_path.read_text(encoding="utf-8")
    except OSError as exc:
        ui.fatal(f"cdcs: cannot read {input_path}: {exc}")
        return None
