...
dev). The fallback keeps ``cdcs-mini`` usable right after ``make
...
# ``src/cdcs_mini``). That keeps the lookup robust against editable
...
def _repo_root() -> Path:
    ...
    raise TypeScriptRuntimeError(
        ...
        "Are you running cdcs-mini outside the source tree?"
    )
